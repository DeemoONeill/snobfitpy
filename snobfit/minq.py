...
__all__ = ['Minq',  # The minq class
           'minq',  # The minq function ( User API )
           ]
...
class Minq:
    ...
    def setSearchInit(self):
        """
        Some initialization for search
        """
    ...
    def _Info(self):
        """ Set the flag indicating the search results """
        self.info = 0
        ...
        elif self.nitref > 0 and \
             self.nfree_old == self.nfree and \
             self.fvalnew   >= self.fval:   # Good termination
    ...
    def ldlrk1(self, L, d, alpha, u):
        """
        Computes LDL^T factorization for
        ...
        Inputs:
        -------
        L     -- A 2D numpy.array. [ If it's a matrix, translate it into
                 numpy.array first.  For example, L = numpy.asarray(L) ]
        ...
        Output:
        ------
        L -- Updated L (2D numpy.array)
        d -- contains diag(D) and is assumed positive
        p -- a direction p of null or negative curvature
        
        Warning:
        --------
        Does not work for the problem of dimension 0
        """
        L = numpy.asarray(L)
        p = numpy.array([])
        if alpha==0:
    ...
    def ldlup(self, L, d, j, g):
        ...
        u    = mldiv(LII, tmat )
        v    = u / d[0:j]
        delta = g[j] - numpy.dot(u, v)
        if  delta <= self.neps:
           pLII = mldiv(LII.T,v)
           p    = numpy.zeros( n, 'd')
           for i in xrange( len(pLII) ):
               p[i] = pLII[i]
           p[j] = -1
           return (L, d, p)
    ...
    def coordinateSearch(self):
        ...
        while 1:
            ...
            # Find step size
            [alpha, lba, uba, self.info] = \
                    self._quadMin( alpha_u, alpha_o, self.gradient[k], q[k] )
    ...
    def subspace(self):
        ...
        for j in find( self.free > self.K ):  # List of newly freed indices
            ...
            p[ind] = self.G[ind,j]
            p[j]   = self.G[j,j]
            [self.L, self.d, pp] = self.ldlup(self.L, self.d, j, p)
        ...
        if  definite:  # Find reduced Newton direction
            ...
            p      = -mldiv( self.L.T, mldiv(self.L,p)/self.d )
    ...
    def calcGain(self):
        """ Calculate gain """
        return self.fval - self.gamma - \
               0.5* numpy.dot( self.x, self.c + toRow(self.gradient) )
    ...
    def calc_newfval(self):
        return self.gamma + \
               0.5 * numpy.dot( self.c + toRow(self.gradient), self.xTry)
    ...
    def search(self):
        ...
        Alternating coordinate and subspace searches
        ...
        info -- Flag indicating search result:
        ...
        while 1:
            ...
            if  self.prt:
                ...
                if  self.unfix and sum(self.nfree) < self.n:
...
def minq( c, G, xlow, xhigh, x0=None, gamma=0, prt=0 ):
    ...
    >>> c = numpy.array([1.0370,3.9598,3.0826])
    >>> G = numpy.matrix([[ 8.6627, 7.5820, 9.2232],
    >>>                 [ 7.5820, 12.0645,8.8799],
    >>>                 [ 9.2232, 8.8799, 11.5346]])
    >>>
    >>> yu = numpy.array([0]*3) - 100000.0
    >>> yo = numpy.array([1000]*3)  + 100000.0
    >>> (x, info) = minq(c, G, yu, yo, 0, 0)
    >>> x
    >>> array([ 1.2659228 , -0.42006664, -0.95610679])
    >>> info
    >>> 0
    """
...
def test2():
    c = numpy.array([21.0370,23.9598,23.0826,10.5767,18.5893,19.8627,16.4453])
    ...
    yu = numpy.array([0]*7   ) - 100000000.0
    yo = numpy.array([1000]*7) + 10000000.0
    m = Minq(0, c, G, yu, yo, 0)
